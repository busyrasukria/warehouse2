...
$page_title = 'Warehouse In'; // Set page title for header
...
try {
    $stmt = $pdo->prepare("
        SELECT 
            wil.*,  -- Select all columns from warehouse_in
            m.part_no_B,
            m.erp_code_B
        FROM warehouse_in wil
...
<div class="container mx-auto px-6 py-10">
    ...
    <div class="bg-gradient-to-r from-blue-800 via-blue-700 to-sky-500 rounded-xl shadow-2xl px-6 py-5 mb-8 text-white animate-fade-in">
        <div class="flex flex-col md:flex-row justify-between items-center gap-4">
            <div>
                <h2 class="text-xl md:text-2xl font-bold flex items-center space-x-3">
                    <i data-lucide="scan-line" class="w-8 h-8"></i>
                    <span>Warehouse In - Scan Ticket</span>
                </h2>
                <p class="text-blue-100 text-sm mt-1">
                    Scan tickets or use "Manual Entry" to log items into the warehouse.
                </p>
            </div>
            ...
            <div class="flex items-center gap-4 w-full md:w-auto">
                ...
                <button id="manualEntryBtn" 
                        ...
                        class="bg-gradient-to-r from-purple-500 to-indigo-600 hover:from-purple-600 hover:to-indigo-700 text-white font-bold py-3 px-6 rounded-lg shadow-lg flex items-center justify-center gap-2 transition-all duration-200 transform hover:scale-105
                               w-48">
        ...
        <form id="scanForm">
            ...
            <div class="relative">
                ...
                <input type="text" id="scanInput" name="qr_data"
                       class="w-full pl-14 pr-4 py-4 border-2 border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-300 shadow-sm hover:shadow-md text-lg"
    ...
    <div class="bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden">
        ...
        <div class="bg-gradient-to-r from-blue-700 via-blue-600 to-sky-500 px-6 py-5 flex flex-col md:flex-row justify-between md:items-center gap-4 shadow-inner">
            ...
            <div class="flex flex-wrap gap-3">
                <div class="relative">
                    ...
                    <select id="modelFilter" class="pl-9 pr-6 py-2 rounded-lg text-sm border border-gray-300 bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none shadow-sm hover:shadow-md">
                ...
                <div class="relative">
                    ...
                    <input type="date" id="dateFilterInput"
                           class="pl-10 pr-4 py-2 rounded-lg text-sm border border-gray-300 text-gray-700 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 outline-none shadow-sm hover:shadow-md w-full md:w-auto" />
                </div>
                <div class="relative">
                    <i data-lucide="search" class="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"></i>
                    <input type="text" id="historySearchInput" 
                           placeholder="Search (TT ID, ERP, Part No, Released By...)"
                           class="pl-10 pr-4 py-2 rounded-lg text-sm border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 outline-none shadow-sm hover:shadow-md w-full md:w-72" />
                </div>
                ...
                <button id="downloadCsvBtn" title="Download CSV"
                    class="p-2 rounded-lg text-sm border border-gray-300 bg-white text-gray-700
        ...
        <div class="overflow-x-auto custom-scrollbar">
            <table id="scanHistoryTable" class="w-full text-sm">
                ...
                <thead class="sticky top-0 z-10"> 
                    <tr>
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider">Date/Time In</th>
                ...
                <tbody id="scanHistoryTableBody" class="bg-white divide-y divide-gray-200">
                    ...
                    <?php foreach ($recentScans as $scan): ?>
                        <tr data-log-id="<?php echo $scan['log_id']; ?>">
                            ...
                            <td class="px-4 py-4">
                                <button 
                                    onclick="deleteScan(<?php echo $scan['log_id']; ?>, this)"
...
<div id="manualEntryModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-start justify-center z-50 opacity-0 invisible transition-opacity duration-300 py-10">
    ...
    <div class="bg-white rounded-3xl shadow-2xl w-full max-w-3xl mx-4 transform scale-95 transition-transform duration-300 hover:scale-100 flex flex-col max-h-full">
        ...
        <div class="bg-gradient-to-r from-purple-600 to-indigo-700 px-8 py-6 rounded-t-3xl flex items-center justify-between flex-shrink-0">
            <h3 class="text-2xl font-bold text-white flex items-center space-x-3">
                <i data-lucide="keyboard" class="w-6 h-6"></i>
                <span>Manual Warehouse In</span>
            </h3>
        ...
        <form id="manualEntryForm" class="p-8 overflow-y-auto">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="text" id="manual_ticket_id" required
                           class="w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all duration-300 shadow-sm hover:shadow-md"
            ...
            <div class="flex items-center justify-end space-x-4 mt-8 pt-6 border-t border-gray-200">
                ...
                <button type="button" id="submitManualEntry"
                        class="px-8 py-3 bg-gradient-to-r from-purple-600 to-indigo-700 hover:from-purple-700 hover:to-indigo-800 text-white font-bold rounded-xl transition-all duration-200 flex items-center space-x-2 shadow-md hover:shadow-lg
...
<script src="assets/warehouse_in.js"></script>
<script>
    lucide.createIcons();
</script>
