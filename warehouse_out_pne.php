...
$page_title = 'Warehouse Out to PNE'; // Set page title for header
...
try {
    ...
    $stmt = $pdo->prepare("
        SELECT 
            wil.*,  -- Select all columns from warehouse_out_pne
            m.part_no_B,
            m.erp_code_B
        FROM warehouse_out_pne wil
...
<div class="container mx-auto px-6 py-10">
    <!-- === THEME CHANGED (AMBER/ORANGE) === -->
    <div class="bg-gradient-to-r from-amber-600 via-red-600 to-orange-400 rounded-xl shadow-2xl px-6 py-5 mb-8 text-white animate-fade-in">
        <div class="flex flex-col md:flex-row justify-between items-center gap-4">
            <div>
                <h2 class="text-xl md:text-2xl font-bold flex items-center space-x-3">
                    <i data-lucide="scan-line" class="w-8 h-8"></i>
                    <span>Warehouse Out to PNE</span>
                </h2>
                <p class="text-orange-100 text-sm mt-1">
                    Scan tickets or use "Manual Entry" to scan items out to PNE.
                </p>
            </div>
        ...
        <form id="scanForm">
            ...
            <div class="relative">
                ...
                <input type="text" id="scanInput" name="qr_data"
                       class="w-full pl-14 pr-4 py-4 border-2 border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all duration-300 shadow-sm hover:shadow-md text-lg"
    ...
    <div class="bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden">
        ...
        <!-- === THEME CHANGED (AMBER/ORANGE) === -->
        <div class="bg-gradient-to-r from-amber-600 via-orange-600 to-red-500 px-6 py-5 flex flex-col md:flex-row justify-between md:items-center gap-4 shadow-inner">
            <h3 class="text-xl font-bold text-white flex items-center tracking-wide drop-shadow-sm">
                <i data-lucide="history" class="w-6 h-6 mr-3 text-orange-200"></i>
                <span>Recent PNE Scan History</span>
            </h3>
            <div class="flex flex-wrap gap-3">
                <div class="relative">
                    <i data-lucide="filter" class="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"></i>
                    <select id="modelFilter" class="pl-9 pr-6 py-2 rounded-lg text-sm border border-gray-300 bg-white focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none shadow-sm hover:shadow-md">
                ...
                <div class="relative">
                    ...
                    <input type="date" id="dateFilterInput"
                           class="pl-10 pr-4 py-2 rounded-lg text-sm border border-gray-300 text-gray-700 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-all duration-200 outline-none shadow-sm hover:shadow-md w-full md:w-auto" />
                </div>
                <div class="relative">
                    <i data-lucide="search" class="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"></i>
                    <input type="text" id="historySearchInput" 
                           placeholder="Search (TT ID, ERP, Part No, Released By...)"
                           class="pl-10 pr-4 py-2 rounded-lg text-sm border border-gray-300 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-all duration-200 outline-none shadow-sm hover:shadow-md w-full md:w-72" />
                </div>
        ...
        <div class="overflow-x-auto custom-scrollbar">
            <table id="scanHistoryTable" class="w-full text-sm">
                ...
                <thead class="sticky top-0 z-10"> 
                    <tr>
                        <!-- === HEADER CHANGED === -->
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider">Date/Time WO to PNE</th>
...
<!-- === NEW MANUAL ENTRY MODAL (PNE THEME) === -->
<div id="manualEntryModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-start justify-center z-50 opacity-0 invisible transition-opacity duration-300 py-10">
    ...
    <div class="bg-white rounded-3xl shadow-2xl w-full max-w-3xl mx-4 transform scale-95 transition-transform duration-300 hover:scale-100 flex flex-col max-h-full">
        ...
        <div class="bg-gradient-to-r from-gray-700 to-gray-800 px-8 py-6 rounded-t-3xl flex items-center justify-between flex-shrink-0">
            <h2 class="text-xl md:text-2xl font-bold flex items-center space-x-3">
                    <i data-lucide="scan-line" class="w-8 h-8"></i>
                    <span>Warehouse Out to PNE In - Scan Ticket</span>
                </h2>
        ...
        <form id="manualEntryForm" class="p-8 overflow-y-auto">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="text" id="manual_ticket_id" required
                           class="w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all duration-300 shadow-sm hover:shadow-md"
            ...
            <div class="flex items-center justify-end space-x-4 mt-8 pt-6 border-t border-gray-200">
                ...
                <button type="button" id="submitManualEntry"
                        class="px-8 py-3 bg-gradient-to-r from-orange-600 to-red-600 hover:from-orange-700 hover:to-red-700 text-white font-bold rounded-xl transition-all duration-200 flex items-center space-x-2 shadow-md hover:shadow-lg
                               disabled:opacity-50 disabled:cursor-not-allowed"
                        disabled>
                    <i data-lucide="save" class="w-5 h-5"></i>
                    <span>Submit Scan Out</span>
                </button>
            </div>
        </form>
    </div>
...
<!-- === SCRIPT CHANGED === -->
<script src="assets/warehouse_out_pne.js"></script>
<script>
    lucide.createIcons();
</script>
