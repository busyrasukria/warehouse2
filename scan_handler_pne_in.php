...
try {
    ...
    if (!$ticket) {
        echo json_encode(['success' => false, 'message' => "Transfer Ticket not found in database."]);
        exit;
    }
    ...
    // 4. CHECK WAREHOUSE OUT PNE
    $stmt_check_out = $pdo->prepare("SELECT 1 FROM warehouse_out_pne WHERE transfer_ticket_id = ? AND erp_code_FG = ?");
    $stmt_check_out->execute([$transfer_ticket_id, $erp_code_from_qr]);
    if (!$stmt_check_out->fetch()) {
        echo json_encode(['success' => false, 'message' => 'Error: Ticket must be scanned OUT to PNE before it can be scanned IN.']);
        exit;
    }
    ...
    // 7. INSERT INTO PNE WAREHOUSE IN
    $stmt_insert = $pdo->prepare("
        INSERT INTO pne_warehouse_in 
    ...
    $pdo->prepare("INSERT INTO ticket_status_log (ticket_id, status_code, status_message, scanned_by) VALUES (?, ?, ?, ?)")
        ->execute([$transfer_ticket_id, 'PNE_IN', 'Returned from PNE to Warehouse', $manpower_display]);

    // 8. SUCCESS
    echo json_encode([
        'success' => true,
        'message' => 'Ticket Scanned IN from PNE!',
        'scanData' => [
            'log_id' => $new_log_id,
    ...
    if ($e->getCode() == '23000') {
        echo json_encode(['success' => false, 'message' => "Error: This ticket has already been scanned IN from PNE."]);
    } else {
        error_log("Scan Handler PNE In Error: " . $e->getMessage());
