document.addEventListener("DOMContentLoaded", () => {
  ...
  const dateFilterInput = $("dateFilterInput"); // <-- ADDED
  ...
  if (dateFilterInput) dateFilterInput.addEventListener("change", () => { currentPage = 1; filterTable(); }); // <-- ADDED
  ...
  if (refreshBtn) {
      refreshBtn.addEventListener("click", () => {
          ...
          if (dateFilterInput) dateFilterInput.value = ""; // <-- ADDED
  ...
  if (fetchDetailsBtn) {
    fetchDetailsBtn.addEventListener("click", () => {
        ...
        fetch("fetch_ticket_details.php", {
            method: "POST",
...
if (downloadCsvBtn) {
    downloadCsvBtn.addEventListener("click", () => {
        ...
        window.location.href = `export/warehouse_in.php?${params.toString()}`;
    });
}